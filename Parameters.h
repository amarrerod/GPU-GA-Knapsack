...
struct TEvolutionParameters {
  ...
  int MaxEvaluations;           // Maximum evaluations to perform
...
/*
 * Singleton class with Parameters maitaining them in CPU and GPU constant
 * memory
 */
class TParameters {
 public:
  ...
  // Destructor
  virtual ~TParameters() { pTParametersInstanceFlag = false; };
  ...
  void SetChromosomeSize(unsigned int Value) {
    EvolutionParameters.ChromosomeSize = Value;
  };

  int MaxEvaluations() const { return EvolutionParameters.MaxEvaluations; };
  ...
  unsigned int CrossoverUINTBoundary() const {
    return EvolutionParameters.CrossoverUINTBoundary;
  };
  unsigned int MutationUINTBoundary() const {
    return EvolutionParameters.MutationUINTBoundary;
  };

  int OffspringPopulationSize() const {
    return EvolutionParameters.OffspringPopulationSize;
  };

  int StatisticsInterval() const {
    return EvolutionParameters.StatisticsInterval;
  };
