...
void TGPU_Statistics::Calculate(TGPU_Population* Population, bool PrintBest) {
  ...
  CalculateStatistics<<<TParameters::GetInstance()->GetGPU_SM_Count() * 2,
                        BLOCK_SIZE>>>(DeviceData, Population->DeviceData,
                                      GPU_Lock);
  ...
  HostData->AvgFitness =
      HostData->AvgFitness / TParameters::GetInstance()->PopulationSize();
  HostData->Divergence = sqrtf(fabs(
      (HostData->Divergence / TParameters::GetInstance()->PopulationSize()) -
 ...
 */
string TGPU_Statistics::GetBestIndividualStr(
    TKnapsackData* GlobalKnapsackData) {
  stringstream S;
  ...
  for (int BitID = 0; BitID < Params->IntBlockSize() -
                                  (GlobalKnapsackData->NumberOfItems -
                                   GlobalKnapsackData->OriginalNumberOfItems);
       BitID++) {
    char c =
        ((HostBestIndividual[Params->ChromosomeSize() - 1] & (1 << BitID)) == 0)
            ? '0'
            : '1';
    S << c;
    if (BitID % 8 == 7) S << " ";
...
void TGPU_Statistics::AllocateCudaMemory() {
  ...
  checkCudaErrors(cudaHostAlloc((void**)&HostData, sizeof(TStatisticsData),
                                cudaHostAllocDefault));

  // Allocate best individual on the host side
  checkCudaErrors(cudaHostAlloc(
      (void**)&HostBestIndividual,
      sizeof(TGene) * TParameters::GetInstance()->ChromosomeSize(),
      cudaHostAllocDefault));

  //------------ Device data ---------------//

  // Allocate data structure on host side
  checkCudaErrors(cudaMalloc((void**)&DeviceData, sizeof(TStatisticsData)));
 ...
 * @param PrintBest  - In false, there's no need to transfer an individual down
 * to host
 *
 */
void TGPU_Statistics::CopyOut(TGPU_Population* Population, bool PrintBest) {
  ...
  checkCudaErrors(cudaMemcpy(HostData, DeviceData, sizeof(TStatisticsData),
...
void TGPU_Statistics::InitStatistics() {
  ...
  // Copy 4 statistics values
  checkCudaErrors(cudaMemcpy(DeviceData, HostData, sizeof(TStatisticsData),
