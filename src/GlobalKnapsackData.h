...
struct TKnapsackData {
  ...
  float MaxPriceWightRatio;   // What is the best Price-Weight ration (to
                              // penalization)
...
class TGlobalKnapsackData {
 public:
  TKnapsackData
      *DeviceData;  // Pointer to Device (GPU) data - necessary for transfers
 ...
 protected:
  ...
  // Handlers on inner arrays of TKnapsack Data on GPU (we cannot copy the
  // structure at once)
