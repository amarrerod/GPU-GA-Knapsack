 ...
 *
 */
TGPU_Population::TGPU_Population(const int PopulationSize,
...
void TGPU_Population::CopyIn(const TPopulationData *HostSource) {
  ...
  // Copy chromosomes
  checkCudaErrors(cudaMemcpy(FHost_Handlers.Population, HostSource->Population,
                             sizeof(TGene) * FHost_Handlers.ChromosomeSize *
                                 FHost_Handlers.PopulationSize,
                             cudaMemcpyHostToDevice));

  // Copy fitness values
  checkCudaErrors(cudaMemcpy(FHost_Handlers.Fitness, HostSource->Fitness,
                             sizeof(TFitness) * FHost_Handlers.PopulationSize,
                             cudaMemcpyHostToDevice));
...
void TGPU_Population::CopyOut(TPopulationData *HostDestination) {
  ...
  // Copy chromosomes --//
  checkCudaErrors(cudaMemcpy(HostDestination->Population,
                             FHost_Handlers.Population,
                             sizeof(TGene) * FHost_Handlers.ChromosomeSize *
                                 FHost_Handlers.PopulationSize,
                             cudaMemcpyDeviceToHost));

  //-- Copy fitnesses --//
  checkCudaErrors(cudaMemcpy(HostDestination->Fitness, FHost_Handlers.Fitness,
                             sizeof(TFitness) * FHost_Handlers.PopulationSize,
                             cudaMemcpyDeviceToHost));
...
void TGPU_Population::CopyOutIndividual(TGene *Individual, int Index) {
  checkCudaErrors(cudaMemcpy(
      Individual,
      &(FHost_Handlers.Population[Index * FHost_Handlers.ChromosomeSize]),
...
void TGPU_Population::CopyDeviceIn(const TGPU_Population *GPUPopulation) {
  if (GPUPopulation->FHost_Handlers.ChromosomeSize !=
      FHost_Handlers.ChromosomeSize) {
    throw std::out_of_range("Wrong chromosome size in CopyIn function");
  }

  if (GPUPopulation->FHost_Handlers.PopulationSize !=
      FHost_Handlers.PopulationSize) {
    throw std::out_of_range("Wrong population size in CopyIn function");
  }

  // Copy chromosomes
  checkCudaErrors(cudaMemcpy(FHost_Handlers.Population,
                             GPUPopulation->FHost_Handlers.Population,
                             sizeof(TGene) * FHost_Handlers.ChromosomeSize *
                                 FHost_Handlers.PopulationSize,
                             cudaMemcpyDeviceToDevice));

  // Copy fintess values
  checkCudaErrors(cudaMemcpy(FHost_Handlers.Fitness,
                             GPUPopulation->FHost_Handlers.Fitness,
                             sizeof(TFitness) * FHost_Handlers.PopulationSize,
                             cudaMemcpyDeviceToDevice));
...
void TGPU_Population::AllocateCudaMemory() {
  ...
  checkCudaErrors(cudaMalloc((void **)&DeviceData, sizeof(TPopulationData)));

  // Allocate Population data
  checkCudaErrors(cudaMalloc((void **)&(FHost_Handlers.Population),
                             sizeof(TGene) * FHost_Handlers.ChromosomeSize *
                                 FHost_Handlers.PopulationSize));

  // Allocate Fitness data
  checkCudaErrors(cudaMalloc((void **)&(FHost_Handlers.Fitness),
                             sizeof(TFitness) * FHost_Handlers.PopulationSize));

  // Copy structure to GPU
  checkCudaErrors(cudaMemcpy(DeviceData, &FHost_Handlers,
                             sizeof(TPopulationData), cudaMemcpyHostToDevice));
...
void TGPU_Population::FreeCudaMemory() {
  ...
  checkCudaErrors(cudaFree(FHost_Handlers.Population));

  // Free Fitness data
  checkCudaErrors(cudaFree(FHost_Handlers.Fitness));
 ...
 *
 */
TCPU_Population::TCPU_Population(const int PopulationSize,
                                 const int ChromosomeSize) {
...
string TCPU_Population::GetStringOfChromosome(const int Idx) {
  ...
  for (int BlockID = 0; BlockID < HostData->ChromosomeSize; BlockID++) {
    for (int BitID = 0; BitID < 32; BitID++) {
      char c =
          ((HostData->Population[Idx * HostData->ChromosomeSize + BlockID] &
            (1 << BitID)) == 0)
              ? '0'
              : '1';
      S << c;
...
void TCPU_Population::AllocateCudaMemory() {
  ...
  checkCudaErrors(cudaHostAlloc(
      (void **)&HostData->Population,
      sizeof(TGene) * HostData->ChromosomeSize * HostData->PopulationSize,
      cudaHostAllocDefault));

  // Allocate fitness on the host side
  checkCudaErrors(cudaHostAlloc((void **)&HostData->Fitness,
                                sizeof(TFitness) * HostData->PopulationSize,
                                cudaHostAllocDefault));
...
void TCPU_Population::FreeCudaMemory() {
  ...
  checkCudaErrors(cudaFreeHost(HostData->Population));

  // Free fitness on the host side
  checkCudaErrors(cudaFreeHost(HostData->Fitness));
