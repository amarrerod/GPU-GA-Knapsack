...
__device__ void TwoRandomINTs(RNG_2x32::ctr_type* RandomValues,
...
inline __device__ int Selection(TPopulationData* ParentsData,
                                unsigned int Random1, unsigned int Random2);
...
inline __device__ void CrossoverOPC(TGene& GeneOffspring1,
                                    TGene& GeneOffspring2, TGene GeneParent1,
                                    TGene GeneParent2,
                                    unsigned int RandomValue);

// Perform Uniform Crossover
inline __device__ void CrossoverUniformFlip(TGene& GeneOffspring1,
                                            TGene& GeneOffspring2,
                                            TGene GeneParent1,
                                            TGene GeneParent2, int BitID);

// Perform BitFlip mutation
inline __device__ void MutationBitFlip(TGene& GeneOffspring1,
                                       TGene& GeneOffspring2,
                                       unsigned int RandomValue1,
                                       unsigned int RandomValue2, int BitID);
 ...
 * Copy constructor
 */
TGPU_Lock::TGPU_Lock(const TGPU_Lock& orig)
    : mutex(orig.mutex),
      reference_counter(orig.reference_counter + 1) {}  // end of TGPU_Lock
...
inline __device__ int Selection(TPopulationData* ParentsData,
  ...
  return (ParentsData->Fitness[Idx1] > ParentsData->Fitness[Idx2]) ? Idx1
                                                                   : Idx2;
}  // Selection
//------------------------------------------------------------------------------
...
inline __device__ void MutationBitFlip(TGene& GeneOffspring1,
                                       ...
                                       unsigned int RandomValue1,
                                       unsigned int RandomValue2, int BitID) {
  // GeneOffspring1 ^= ((unsigned int)(RandomValue1 <
  // GPU_EvolutionParameters.MutationUINTBoundary) << BitID); GeneOffspring2 ^=
  // ((unsigned int)(RandomValue2 < GPU_EvolutionParameters.MutationUINTBoundary)
  // << BitID);
  if (RandomValue1 < GPU_EvolutionParameters.MutationUINTBoundary)
    GeneOffspring1 ^= (1 << BitID);
  if (RandomValue2 < GPU_EvolutionParameters.MutationUINTBoundary)
    GeneOffspring2 ^= (1 << BitID);
...
__global__ void GeneticManipulationKernel(TPopulationData* ParentsData,
  ...
  if ((threadIdx.y == 0) && (threadIdx.x < CHR_PER_BLOCK)) {
    ...
    Parent1_Idx[threadIdx.x] =
        Selection(ParentsData, RandomValues.v[0], RandomValues.v[1]);
    Parent2_Idx[threadIdx.x] =
        Selection(ParentsData, RandomValues.v[2], RandomValues.v[3]);

    counter.incr();
    RandomValues = rng_4x32(counter, key);
    CrossoverFlag[threadIdx.x] =
        RandomValues.v[0] < GPU_EvolutionParameters.CrossoverUINTBoundary;
  }
  ...
  while (GeneIdx < GPU_EvolutionParameters.ChromosomeSize) {
    TGene GeneParent1 =
        ParentsData->Population[GetIndex(Parent1_Idx[threadIdx.y], GeneIdx)];
    TGene GeneParent2 =
        ParentsData->Population[GetIndex(Parent2_Idx[threadIdx.y], GeneIdx)];
    ...
    if (CrossoverFlag[threadIdx.y]) {
      counter.incr();
      RandomValues = rng_4x32(counter, key);
      CrossoverUniformFlip(GeneOffspring1, GeneOffspring2, GeneParent1,
                           GeneParent2, RandomValues.v[0]);
    ...
    //-- mutation --//
    for (int BitID = 0; BitID < GPU_EvolutionParameters.IntBlockSize;
         BitID += 2) {
      counter.incr();
      RandomValues = rng_4x32(counter, key);

      MutationBitFlip(GeneOffspring1, GeneOffspring2, RandomValues.v[0],
                      RandomValues.v[1], BitID);
      MutationBitFlip(GeneOffspring1, GeneOffspring2, RandomValues.v[2],
                      RandomValues.v[3], BitID + 1);

    }  // for

    OffspringData->Population[GetIndex(ChromosomeIdx, GeneIdx)] =
        GeneOffspring1;
    OffspringData->Population[GetIndex(ChromosomeIdx + 1, GeneIdx)] =
        GeneOffspring2;
...
__global__ void ReplacementKernel(TPopulationData* ParentsData,
  ...
  if (threadIdx.x == 0) {
    ...
    OffspringIdx_SHM[threadIdx.y] =
        RandomValues.v[0] % (GPU_EvolutionParameters.OffspringPopulationSize);
  }

  __syncthreads();

  //------- replacement --------//
  if (ParentsData->Fitness[ChromosomeIdx] <
      OffspringData->Fitness[OffspringIdx_SHM[threadIdx.y]]) {
    //-- copy data --//
    while (GeneIdx < GPU_EvolutionParameters.ChromosomeSize) {
      ParentsData->Population[GetIndex(ChromosomeIdx, GeneIdx)] =
          OffspringData
              ->Population[GetIndex(OffspringIdx_SHM[threadIdx.y], GeneIdx)];
      GeneIdx += WARP_SIZE;
    }

    if (threadIdx.x == 0)
      ParentsData->Fitness[ChromosomeIdx] =
          OffspringData->Fitness[OffspringIdx_SHM[threadIdx.y]];
...
__global__ void CalculateKnapsackFintess(TPopulationData* PopData,
  ...
  for (int IntBlockIdx = 0;
       IntBlockIdx < GPU_EvolutionParameters.ChromosomeSize; IntBlockIdx++) {
    //--------------Load Data -------------//
    if (threadIdx.y == 0) {
      PriceGlobalData_SHM[GeneInBlockIdx] =
          GlobalData
              ->ItemPrice[IntBlockIdx * GPU_EvolutionParameters.IntBlockSize +
                          GeneInBlockIdx];
      WeightGlobalData_SHM[GeneInBlockIdx] =
          GlobalData
              ->ItemWeight[IntBlockIdx * GPU_EvolutionParameters.IntBlockSize +
                           GeneInBlockIdx];
    }

    ActGene = ((PopData->Population[GetIndex(ChromosomeIdx, IntBlockIdx)]) >>
               GeneInBlockIdx) &
              TGene(1);
    ...
    PriceValues_SHM[threadIdx.y][GeneInBlockIdx] +=
        ActGene * PriceGlobalData_SHM[GeneInBlockIdx];
    WeightValues_SHM[threadIdx.y][GeneInBlockIdx] +=
        ActGene * WeightGlobalData_SHM[GeneInBlockIdx];
  }
  ...
  if (threadIdx.x == 0) {
    ...
    if (WeightValues_SHM[threadIdx.y][0] > GlobalData->KnapsackCapacity) {
      TFitness Penalty =
          (WeightValues_SHM[threadIdx.y][0] - GlobalData->KnapsackCapacity);
